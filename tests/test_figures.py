...
def test_vector_outputs_are_byte_reproducible(tmp_path: Path) -> None:
    ...
    plot_model_comparison(data, first)
    plot_model_comparison(data, second)

    for suffix in ("pdf", "svg", "png"):
        first_hash = hashlib.sha256((first / f"model_comparison.{suffix}").read_bytes())
        second_hash = hashlib.sha256((second / f"model_comparison.{suffix}").read_bytes())
